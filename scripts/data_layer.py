...
def run(args: argparse.Namespace) -> None:
	...
	canonical_train_df = train_split_df[canonical_train_cols].copy()
	canonical_train_df["provenance"] = "gold_train_doc"
	canonical_train_df["trust_weight"] = 1.0
	canonical_train_df.to_csv(output_dir / "canonical_train_pairs.csv", index=False)
	...
	if sentence_path.exists():
		...
		sentence_df["translation_clean"] = sentence_df["translation"].map(normalize_translation)
		sentence_df["first_word_transcription_clean"] = sentence_df["first_word_transcription"].map(
			normalize_transliteration
		)

		sentence_id_col = "text_uuid" if "text_uuid" in sentence_df.columns else sentence_df.columns[0]
		sentence_split_df = deterministic_split_by_id(sentence_df, sentence_id_col, args.valid_ratio, args.split_salt)
		sentence_split_df["provenance"] = "gold_sentence_targets"
		sentence_split_df["trust_weight"] = 1.0
		...
		keep_sentence_cols = [
			...
			if c in sentence_split_df.columns
		]
		sentence_targets = sentence_split_df[keep_sentence_cols].copy()
		...
		train_bridge_df = train_split_df[train_bridge_cols].copy().rename(columns={"oare_id": "text_uuid", "split": "doc_split"})

		sentence_bridge = sentence_targets.merge(train_bridge_df, on="text_uuid", how="left")
		sentence_bridge["has_matching_train_doc"] = sentence_bridge["transliteration"].notna().astype(int)
